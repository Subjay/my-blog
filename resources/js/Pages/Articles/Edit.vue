...
function updateArticle() {
  ...
  const newArticle = {
    ...
    cover: form.cover,
    updated_at: modificationDate.toISOString(),
  }

  form.patch(route('articles.update', newArticle), { onSuccess: () => form.reset(), onError: (err) => { displayAPIErrors(err) } })
}

function submitForm() {
  if( isFormValid() ) {
    form.post(route('upload-cover', props.article.id), { onError: (err) => { displayAPIErrors(err) }, onSuccess: () => { updateArticle() } });
  }
  else {
    displayErrors();
  }
...
<template>
    <Head title="Mise à jour d'un article" />
 
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Modification de l'article</h2>
        </template>
        <div class="mx-auto p-4 sm:p-6 lg:p-8">
            <form @submit.prevent="submitForm" class="flex flex-col">
              ...
              <select v-model="form.category">
                ...
                <option value="Bien-&ecirc;tre">Bien &ecirc;tre</option>
              ...
              <div v-else class="flex flex-col justify-center items-center">
                ...
                <div v-if="!form.imageLoading" class="relative size-1/3">
                  <img v-if="form.coverFile != null" :src="form.imagePreview" />
                  <img v-else :src="'/resources/articles/' + props.article.id + '/' + form.cover" />
              ...
              <PrimaryButton class="mt-4">ENREGISTRER MODIFICATIONS</PrimaryButton>
            </form>
        </div>
    </AuthenticatedLayout>
</template>
